...
# ====== Cấu hình cơ bản ======
APP_NAME="my-streamlit"                        # tên container (theo docker ps)
APP_PORT=8501                                  # port app streamlit trong container
REPO_URL="https://github.com/truongquangphuc/new-766"  # link repo mới
APP_DIR="/home/administrator/new-766"          # thư mục source trên server
...
if [ -f "docker-compose.yml" ] || [ -f "docker-compose.yaml" ] || [ -f "compose.yaml" ]; then
  ...
  docker build --no-cache -t my-streamlit-app .   # build image với tên gốc bạn đang dùng
  docker run -d \
    --name "$APP_NAME" \
    -p $APP_PORT:8501 \
    --restart=always \
